...
sample$order<-1:length(sample$V1)
plot.admixture<-function(directory){
  ...
  parse.Q.files<-function(x){
    ...
    dat2$Krun<-ldat
    datt<-merge(dat2,sample,by.x = "names", by.y = "V1")
    colnames(datt)<-c("Name","ov","Admixture.Group","Value", "Krun","order")
    return(datt)
  }	
  plotgg<-function(datframe){
    my.max.k<-max(as.numeric(as.character(datframe$Admixture.Group)))
    datframe<-datframe[order(datframe$ov),]
    datframe$Name<-factor(datframe$Name, ordered=TRUE)
    my.col<-colors()[c(26,547,498,69,33,51,536,100,76,200,300,400,450)]
    the.plot<-ggplot(datframe, aes(x=reorder(Name, order), y=Value, fill=Admixture.Group))+geom_bar(stat="identity")
...
results<- plot.admixture(args[1])

n=1;
for(i in results){
  name<-paste("K",n, ".png", sep="")
  png(name, width = 10, height = 5, units = "in", res = 300)
  print (i)
  dev.off()
  n=n+1
}